...
class AuthRepository {
  ...
  Future<LoginResponse> getSocialLoginResponse(@required String social_provider,
      ...
      {access_token = "",secret}) async {
    var post_body = jsonEncode(
        {"name": "${name}", "email": email, "provider": "$provider","social_provider":"$social_provider","access_token":"$access_token","secret":"$secret"});
    ...
    final response = await http.post(url,
        ...
        body: post_body);
    print("ahmed $post_body");
    print("osama ${response.body.toString()} ");
    return loginResponseFromJson(response.body);
  }
