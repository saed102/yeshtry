...
class PaymentRepository {
  ...
  Future<PaypalUrlResponse> getPaypalUrlResponse(@required String payment_type,
    ...
    Uri url = Uri.parse(
      "${AppConfig.BASE_URL}/paymob/pay?payment_type=${payment_type}&combined_order_id=${combined_order_id}&amount=${amount}&user_id=${user_id.$}",
    );
    final response = await http.get(url, headers: {
      "App-Language": app_language.$,
      "Authorization": "Bearer ${access_token.$}"
...
Future<PaypalUrlResponse> rePayorder(
      @required int order_id, ) async {
    Uri url = Uri.parse(
      "${AppConfig.BASE_URL}/repayment_order/${order_id}",
    );
    final response = await http.get(url, headers: {
      "App-Language": app_language.$,
      "Authorization": "Bearer ${access_token.$}"
    ...
    print("repattttt  ${response.body.toString()}");
    return paypalUrlResponseFromJson(response.body);
  }Future<PaypalUrlResponse> rePayorderst(
      @required int order_id, ) async {
    Uri url = Uri.parse(
      "${AppConfig.BASE_URL}/repayment_order/${order_id}",
    );
